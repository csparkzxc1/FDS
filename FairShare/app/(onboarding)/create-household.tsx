...
import { useCreateHousehold } from '@/hooks/queries/useHousehold';
...
const MODE_OPTIONS: Array<{
  ...
  { mode: 'couple', emoji: '💑', title: '커플 / 동거', desc: '둘이서 함께 가사를 분담해요', color: Colors.primary[50] },
  { mode: 'family', emoji: '👨‍👩‍👧', title: '가족', desc: '부모와 아이가 함께, 용돈 교육도 해요', color: Colors.kid[50] },
  { mode: 'roommate', emoji: '🏘️', title: '룸메이트', desc: '여러 명이 함께 공동 생활을 해요', color: Colors.success[50] },
];

const POINT_RATES = [50, 100, 200, 500];
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        ...
        >
          ...
          {selectedMode === 'family' && (
            <View className="mb-6">
              <Text className="text-sm font-medium text-gray-700 mb-1">포인트 환율</Text>
              <Text className="text-xs text-gray-400 mb-3">
                1포인트 = <Text className="font-semibold text-primary-600">{pointRate}원</Text>
              </Text>
              <View className="flex-row gap-2">
                {POINT_RATES.map((rate) => (
                  <TouchableOpacity
                    key={rate}
                    onPress={() => setPointRate(rate)}
                    activeOpacity={0.7}
                    className="flex-1"
                  >
                    <View
                      ...
                      style={{
                        backgroundColor: pointRate === rate ? Colors.primary[500] : Colors.white,
                        borderColor: pointRate === rate ? Colors.primary[500] : Colors.gray[200],
                      }}
                    >
                      <Text
                        className="text-sm font-semibold"
                        style={{ color: pointRate === rate ? Colors.white : Colors.gray[700] }}
                      >
                        {rate}원
                      </Text>
                    </View>
